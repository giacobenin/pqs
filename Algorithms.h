...
/*
    ...
    T * Algorithms<T>::dijskstra (Graph<T> *pGraph, unsigned int uiSrc, PQueue *pQueue)
    {
        ...
        for (unsigned int i = 0; i < pGraph->getVertexCount(); i++) {
            Vertex<T> *pVertex = pGraph->getVertex (i);
            if (pVertex != NULL) {
                T cost = (pVertex->uiVertexKey == uiSrc ? 0 : INFINITY);
                pNodeDistancePairs[i] = pQueue->add (new NodeDistancePairClass (pVertex->uiVertexKey, cost));
            }
            pVisited[i]  = false;
